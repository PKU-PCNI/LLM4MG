...
class MultiheadAttention(nn.Module):
    def __init__(self, embed_dims, num_heads, dropout=0.0, batch_first=True):
        super().__init__()
        self.attention = nn.MultiheadAttention(embed_dims, num_heads, dropout=dropout, batch_first=batch_first)
...
class RFNLayer(nn.Module):
    def __init__(self, in_channels, out_channels, norm_cfg=None, last_layer=False):
        ...
        self.norm = nn.BatchNorm1d(self.units, eps=1.0e-3, momentum=0.01)
...
class RadarBEVNet(nn.Module):
    def __init__(
        ...
        self.classifier = nn.Sequential(
            ...
            nn.ReLU(inplace=True),
            nn.Dropout(p=drop),
            nn.Linear(feat_channels[-1] // 2, num_classes)  # number of output classes
        )
    ...
    def forward(self, features, num_voxels, coors):
        ...
        if not self.return_rcs:
            # return c.squeeze()
            return self.classifier(c.squeeze())
        else:
            rcs = (rcs_features*mask).sum(dim=1)/mask.sum(dim=1)
            # return c.squeeze(), rcs.squeeze()
            return self.classifier(c.squeeze()), rcs.squeeze()
