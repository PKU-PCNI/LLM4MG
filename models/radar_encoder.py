...
class DMSA(nn.Module):
    def __init__(self, embed_dims=256, num_heads=8, dropout=0.1):
        super().__init__()
        self.attention = MultiheadAttention(embed_dims, num_heads, dropout)
    ...
    def inner_forward(self, query_bbox, query_feat, pre_attn_mask):
        ...
        # use PyTorch MultiheadAttention on permuted tensor
        query_feat_permuted = query_feat.permute(1, 0, 2)
        output, _ = self.attention(
            query=query_feat_permuted,
            key=query_feat_permuted,
            value=query_feat_permuted,
            attn_mask=attn_mask,
        )
        output = output.permute(1, 0, 2)  # restore to [batch, seq_len, embed_dim]
        return output
...
class RadarBEVNet(nn.Module):
    def __init__(
        ...
        self.feat_channels_t = feat_channels
    ...
    def forward(self, features, num_voxels, coors):
        ...
        # Ensure features is 3D: [batch_size, num_points, channels]
        if features.ndim == 2:
            features = features.unsqueeze(0)
        if num_voxels.ndim == 0:
            num_voxels = num_voxels.unsqueeze(0)
        if coors.ndim == 2:
        ...
        x_coord = coors[:, :, 0].to(dtype).unsqueeze(-1)
        y_coord = coors[:, :, 1].to(dtype).unsqueeze(-1)
        x_offset_center = self.x_offset - x_coord * self.vx
        y_offset_center = self.y_offset - y_coord * self.vy
        f_center[:, :, 0] = features[:, :, 0] - x_offset_center.squeeze(-1)  # assume x_offset_center is (batch_size, num_voxels)
        f_center[:, :, 1] = features[:, :, 1] - y_offset_center.squeeze(-1)
        ...
        mask_without_channel = mask.squeeze(-1)
        features_mean[:, :, 0] = features[:, :, 0] - ((features[:, :, 0] * mask_without_channel).sum(dim=1) / mask_without_channel.sum(dim=1)).unsqueeze(1)
        features_mean[:, :, 1] = features[:, :, 1] - ((features[:, :, 1] * mask_without_channel).sum(dim=1) / mask_without_channel.sum(dim=1)).unsqueeze(1)
        ...
        if batch_size > 1:
            bs_list = [features.shape[1] for _ in range(coors.shape[0])]
        elif batch_size == 1:
            bs_list = [features.shape[1]]
        else:
            assert False

        points_coors = points_coors.flatten(start_dim=0, end_dim=1)
        ...
        for rfn in self.point_block:
            ...
            x = rfn(x)
            x_t = x.flatten(start_dim=0, end_dim=1)
            x_split = torch.split(x_t, bs_list)

            c_t = c.flatten(start_dim=0, end_dim=1)
            c_split = torch.split(c_t, bs_list)
            ...
            for bs in range(len(x_split)):
                ...
                c_tmp = c_tmp + self.injector[i](self.compress(c_tmp), self.compress(x_tmp)).max(dim=0)[0].expand_as(c_tmp)
                x_tmp = x_tmp + self.extractor[i](self.compress(x_tmp), self.compress(c_tmp)).max(dim=0)[0].expand_as(x_tmp)
                c_tmp = self.transformer_block[i](self.compress(c_tmp), self.compress(points_coors_tmp)).max(dim=0)[0].expand_as(c_tmp)
            ...
            x = torch.cat(x_out_list, dim=0).reshape(len(bs_list), bs_list[0], x.shape[2])
            c = torch.cat(c_out_list, dim=0).reshape(len(bs_list), bs_list[0], self.feat_channels_t[i+1 if i<len(self.feat_channels_t)-1 else i])
            i += 1


        c = self.out_linear(torch.cat([c, x], dim=-1))
        batch_size, dim, num_points = c.shape
        c = c.reshape(batch_size, dim, 16, 16)
